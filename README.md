...
Set `ORBIEN_TOKEN` and `DASHBOARD_PASSWORD` during installation. The dashboard defaults to user `admin`; the deployment credentials are sent as an HTTP Basic Auth header so the browser login prompt is handled automatically.
